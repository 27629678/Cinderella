...
local ch1Scene3 = class("ch1Scene3", function()
    return display.newScene("ch1Scene3")
end)

function ch1Scene3:ctor()
    local ch_id       = 1
    local ch_index    = 2
    ...
    -- 鸟
    self.birds = uianimationex:createAnimationWithArmature("sanzhiniao", CCPoint(400, 100), self.animlayer)
    ...
    publicmenu:addUI({
        ...
        listener_right  = function(index)
                                if index == 1 then
                                    ...
                                    uianimationex:playAnim(self.birds, "gezi")
                                elseif index == 2 then
                                    uianimationex:playAnim(self.cinderella, "xiao")
                                    audio.playMusic(string.format(SoundPath, "Cinderella_3.MP3"), false)
                                    uianimationex:playAnim(self.birds, "maque")
                                elseif index == 3 then
                                    uianimationex:playAnim(self.cinderella, "xiao")
                                    audio.playMusic(string.format(SoundPath, "Cinderella_3.MP3"), false)
                                    uianimationex:playAnim(self.birds, "jizhiniao")
                                else
                                    -- do nothing
                                end
                            end,
...
function ch1Scene3:onEnterFrame(dt)
...
function ch1Scene3:onQuestionButtonClicked(event)
    print "禁用角色触摸事件"
    self.stepmother:setTouchEnabled(false)
    self.cinderella:setTouchEnabled(false)
end

function ch1Scene3:onRewindButtonClicked(event)
